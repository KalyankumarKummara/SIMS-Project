...
const EditInternship = () => {
  ...
  // Fetch internship details when the component mounts
  useEffect(() => {
    const fetchInternshipDetails = async () => {
      try {
        ...
        console.log("Fetched Internship Data:", internshipResult); // Debugging: Log fetched data
        ...
        console.log("Fetched Company Data:", companyResult); // Debugging: Log fetched company data

        // Map backend response to frontend formData structure
        setFormData({
          company_id: data.company_id || "",
          company_name: companyResult.data.name || "Unknown Company", // Use company name from company details
          title: data.title || "",
          type_of_internship: data.type_of_internship || "",
          description: data.description || "",
          required_skills: data.required_skills || [],
          location: data.location || "",
          duration: data.duration || "",
          mode_of_internship: data.mode_of_internship || "",
          open_positions: data.open_positions || "",
          application_deadline: data.application_deadline || "",
          created_date: data.created_date || new Date().toISOString().split("T")[0], // Include created_date
          ...
          internship_status: data.internship_status || "Pending",
          contact_email: data.contact_email || "", // Include contact_email
  ...
  // Handle changes for multi-select dropdowns
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const payload = {
        company_id: formData.company_id, // Include company_id
        company_name: formData.company_name, // Include company_name
        title: formData.title,
        type_of_internship: formData.type_of_internship,
        description: formData.description,
        required_skills: formData.required_skills,
        location: formData.location,
        duration: formData.duration,
        mode_of_internship: formData.mode_of_internship,
        open_positions: parseInt(formData.open_positions, 10),
        application_deadline: formData.application_deadline,
        created_date: formData.created_date, // Include created_date
        benefits: formData.benefits,
        application_process: formData.application_process,
        stipend_type: formData.stipend_type,
        stipend_min: parseFloat(formData.stipend_min),
        stipend_max: parseFloat(formData.stipend_max),
        internship_status: formData.internship_status,
        contact_email: formData.contact_email, // Include contact_email
      ...
      if (!response.ok) {
        const errorText = await response.text(); // Debugging: Log the error response
      ...
      if (result.status === "success") {
        ...
        setTimeout(() => {
          navigate("/recruiter-dashboard"); // Redirect to the recruiter dashboard after 3 seconds
        }, 3000);
      } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-blue-50 to-purple-50">
      ...
      {!isSuccess && <CompanyNavbar pageTitle="Edit Internship" />}

      {/* Main Content */}
      <div className="flex justify-center items-center min-h-screen mt-4 mb-8">
        <motion.div
         className="w-full max-w-4xl mx-auto p-8 bg-white rounded-lg text-gray-900 border border-gray-300 shadow-xl"
        ...
        >
          {isSuccess ? (
            <div className="text-center">
              ...
              <p className="text-xl font-semibold text-gray-800">Internship Updated Successfully!</p>
            </div>
          ) : (
            <>
              <motion.h2
